...
module.exports = function(server, datapackage , datapackageV2) {
  ...
  /**
   * Validates a CSV resource data file For Version 2.
   */
  server.route({
    path: '/v2/validate/csv',
    method: 'POST',
    config: {
      tags: ['api'],
      description: 'Validate a CSV data file using a specific Open Referral resource schema',
      ...
      async handler(request, h) {
        ...
        try {
          ...
          const result = await datapackageV2.validateResource(stream, type);
          let csvPath = path.join(process.cwd(), "public/"+( (new Date()).getTime())+".csv" );
 ...
 /**
   * Validates a ZIP resource data file For Version 2.
   */

  server.route({
    path: '/v2/validate/zip',
    method: 'POST',
    config: {
      tags: ['api'],
      description: 'Validate a CSV data file using a specific Open Referral resource schema',
      ...
      async handler(request, h) {
        ...
        try {
          ...
          const _files = [
            ...
            { name: "postal_address", status: "No Data", result: "-" , filename: 'postal_addresses', class: ''},
            { name: "program", status: "No Data", result: "-" , filename: 'programs', class: ''},
            { name: "schedule", status: "No Data", result: "-" , filename: 'schedules', class: ''},
            { name: "required_document", status: "No Data", result: "-" , filename: 'required_documents', class: ''},
            { name: "service_area", status: "No Data", result: "-" , filename: 'service_areas', class: ''},
            { name: "service", status: "No Data", result: "-" , filename: 'services', class: ''},
            { name: "service_at_location", status: "No Data", result: "-" , filename: 'services_at_location', class: ''},
            { name: "service_attribute", status: "No Data", result: "-" , filename: 'service_attribute', class: ''},
            { name: "taxonomy_term", status: "No Data", result: "-" , filename: 'taxonomy_term', class: ''},
          ...
          for(let f of _files) {
            try {
              ...
              let csvPath = path.join(dirPath, cName+".csv");
              if (fs.existsSync(csvPath)) {
                resultArr[cName] = await datapackageV2.validateResource(csvPath, f.name);
              }
            } catch(err) {
              console.error(err)
            }
